...
local function check_equipment(item, id, item_info, settings)
    ...
    if rarity >= 5 then
        ...
        local required_ga_count = settings.ga_count
...
local function check_stackable_logic(enabled, full_check_func, get_items_func, item_id, max_stack, current_stack)
    ...
    return Utils.is_lowest_stack_below(get_items_func(), item_id, max_stack, current_stack)
end
...
function ItemManager.get_pick_reason_with_flags(item, ignore_distance, ignore_inventory, item_info)
   ...
   if not item_info then
      ...
      local ok_info, info = pcall(function() return item:get_item_info() end)
      if not ok_info then return false, "Info Error", nil end
      item_info = info
   end
   
   if not item_info or not item_info:is_valid() then return false, "Invalid Info", nil end
...
function ItemManager.check_want_item(item, ignore_distance, ignore_inventory, item_info)
   local wanted, reason, info = ItemManager.get_pick_reason_with_flags(item, ignore_distance, ignore_inventory, item_info)
   return wanted, info
end
...
function ItemManager.calculate_item_score(item, item_info)
   ...
   if not item_info then
      local ok_info, info = pcall(function() return item:get_item_info() end)
      if not ok_info or not info then return 0 end
      item_info = info
   end
